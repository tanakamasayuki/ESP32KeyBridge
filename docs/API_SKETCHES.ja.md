...
```cpp
esp32keybridge::InputCode code = esp32keybridge::keyboardCode(esp32keybridge::KeySymbol::A);
```
...
`esp32keybridge::TransformConfig` の remap / disable は `esp32keybridge::InputCode` ベースです。`esp32keybridge::KeySymbol` を受け取る API は keyboard domain 用の convenience として扱います。
...
```cpp
esp32keybridge::InputEvent event = esp32keybridge::keyEvent(esp32keybridge::KeySymbol::A, true);
```
...
void loop()
{
  input.apply(esp32keybridge::keyEvent(esp32keybridge::KeySymbol::CapsLock, true));
  bridge.update();
}
```
...
void loop()
{
  keyboardA.apply(esp32keybridge::keyEvent(esp32keybridge::KeySymbol::LeftShift, true));
  keyboardB.apply(esp32keybridge::keyEvent(esp32keybridge::KeySymbol::A, true));
  bridge.update();
}
```
...
keyboard.remap(esp32keybridge::KeySymbol::CapsLock, esp32keybridge::KeySymbol::LeftCtrl);

auto scanner = config.input(1);
scanner.remap(esp32keybridge::KeySymbol::Enter, esp32keybridge::KeySymbol::Tab);
...
config.global.remap(esp32keybridge::KeySymbol::CapsLock, esp32keybridge::KeySymbol::LeftCtrl);
config.global.remap(esp32keybridge::KeySymbol::LeftCtrl, esp32keybridge::KeySymbol::CapsLock);
...
config.global.macro(esp32keybridge::KeySymbol::Fn1, macroKeys, 3);
...
意味キーの remap とは別に、USB HID usage と意味キーの decode / encode は `esp32keybridge::KeyboardLayout` で扱います。

```cpp
esp32keybridge::KeySymbol symbol =
    esp32keybridge::KeyboardLayout::fr().decode(esp32keybridge::HidUsage::Usage04);

esp32keybridge::HidUsage outputUsage =
    esp32keybridge::KeyboardLayout::us().encode(symbol);
```

この例では FR layout の HID usage `0x04` は `esp32keybridge::KeySymbol::Q` として解釈され、US layout で出力すると HID usage `0x14` になります。`esp32keybridge::KeySymbol::Q` は意味としての Q であり、HID usage `0x14` そのものではありません。

layout conversion 後の意味キーに対して、さらに key mapping table を適用する使い方もできます。
...
現在の `esp32keybridge::LayoutConfig` は汎用的な key mapping table として扱います。US/FR などの実 layout 定義、文字意味ベースの decode / encode は `esp32keybridge::KeyboardLayout` 側に置きます。

USB Host adapter は Host 側 layout を指定せず、raw HID usage を受け取ります。文字意味ベースの変換が必要な場合は、bridge 側で以下のような段階に分けます。

```text
HID usage
  -> input layout decode
  -> logical key meaning
  -> output layout encode
  -> output HID usage + modifiers
```

最小 example は [examples/LayoutConversion](../examples/LayoutConversion/README.ja.md) に置きます。
...
state.press(esp32keybridge::KeySymbol::A);
