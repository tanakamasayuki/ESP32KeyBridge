...
7. **アダプタと example**: USB Host keyboard/mouse 入力、USB device(複合)出力、GPIO、文字列デバイス(シリアル)。既存の examples / tests を新 API へ移行。**(進行中: USB Host keyboard/mouse 入力・USB Device 複合 HID 出力・GPIO 入力・GPIO マトリクス・ロータリーエンコーダは実装済みで実機検証待ち。BLE は専用ライブラリの完成待ちでこのライブラリでは当面非対応 → [DECISIONS.ja.md](DECISIONS.ja.md))**
