...
7. **アダプタと example**: USB Host keyboard/mouse 入力、USB device(単体 / 複合)出力、GPIO マトリクス、BLE、文字列デバイス(シリアル)。既存の examples / tests を新 API へ移行。**(進行中: examples と、スケッチ向け API を確定するビルド専用モックアダプタを先行配置。実装は SwapCtrlCapsLock に必要な USB Host 入力 / USB Device 出力から)**
