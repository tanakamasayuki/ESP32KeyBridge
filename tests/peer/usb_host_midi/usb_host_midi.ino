// Peer smoke test, host board: the USB Host MIDI input adapter feeds the
// bridge input pipeline. MIDI notes are mapped to keyboard keys so the
// result shows up as keys in the output set. The peer device board acts as
// the USB MIDI device (peer_device/peer_device.ino).
//
...
void setup()
{
  ...
  // Notes 60/62 map to distinct keyboard keys so the test can read them back.
  midi.mapNote(60, esp32keybridge::KeyboardUsage::A); // 0x04
  midi.mapNote(62, esp32keybridge::KeyboardUsage::B); // 0x05
