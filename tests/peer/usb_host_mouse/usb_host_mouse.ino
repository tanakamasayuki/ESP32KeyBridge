// Peer smoke test, host board: the USB Host mouse input adapter feeds the
// bridge input pipeline. Mouse buttons arrive as MouseButton keys in the
// output set; X/Y/wheel arrive as relative axis deltas. The peer device
// board acts as the USB mouse (peer_device/peer_device.ino).
//
// Prints "OUT <kind>:<code>..." (sorted) whenever the button set reaching
// the output adapter changes, and "AXIS x=<dx> y=<dy> wheel=<dw>" on any
// frame that carries a nonzero relative delta.
...
void loop()
{
  ...
  const int32_t dx = bridge.axisDelta(esp32keybridge::Axis::X);
  const int32_t dy = bridge.axisDelta(esp32keybridge::Axis::Y);
  const int32_t dw = bridge.axisDelta(esp32keybridge::Axis::Wheel);
  if (dx != 0 || dy != 0 || dw != 0)
  {
    Serial.printf("AXIS x=%d y=%d wheel=%d\n", static_cast<int>(dx), static_cast<int>(dy),
                  static_cast<int>(dw));
  }
